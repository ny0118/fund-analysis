...
def main():
    print("=" * 60)
    print(f"多品种多周期技术分析（核心品种） - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 60)
    ...
    for i, asset in enumerate(ASSETS):
        ...
        # 请求间延迟 1 秒（GitHub Actions 环境更容易限流）
        if i < len(ASSETS) - 1:
            time.sleep(1)

    # 输出结果（使用相对路径，适配 GitHub Actions 工作目录）
    output_path = "./multi-asset-technical-analysis/scripts/result.json"
